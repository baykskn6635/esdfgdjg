...
.setLabel(Config.Guild.guildName)
.setCustomId("buton3")
...
.addComponents([buton3])

    if (!message.guild || message.author.bot) return
    if (["tag", "TAG", "Tag", ".tag", "!tag"].some(x => message.content === x)) {
        message.reply({ content:`\`${Config.Guild.guildTag}\``}) 
        message.channel.send({ content: `\`#${Config.Guild.guildDiscrim}\``, components: [row]})
    }
    ...
    const embed = new EmbedBuilder().setColor(message.member.displayHexColor).setAuthor({name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true, size: 2048 })}).setFooter({ text: `Developed by Felixar`, iconURL: Felixar.displayAvatarURL({dynamic: true})})
